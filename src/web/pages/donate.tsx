...
import { Heart, ArrowLeft, CheckCircle2, PartyPopper } from "lucide-react";
...
export default function Donate() {
        ...
        const handleDonate = async () => {
                if (!session) {
                        setLocation("/sign-in?callbackUrl=/donate");
                        return;
                }
                ...
                try {
                        let productId = "";
                        if (amount) {
                                productId = `donate_${amount}${frequency === "monthly" ? "_monthly" : ""}`;
                        } else if (customAmount) {
                                // For custom amount, we use donate_custom
                                // Note: Autumn might need specific support for variable pricing
                                // For now we map to the nearest tier or use donate_custom
                                productId = "donate_custom";
                        }
                ...
                return (
                        <main className="min-h-screen bg-gray-50 flex items-center justify-center p-6">
                                <div className="max-w-md w-full bg-white rounded-[2.5rem] p-12 text-center shadow-2xl shadow-charcoal/5 border border-gray-100">
                                        ...
                                        <h1 className="text-3xl font-serif font-black text-charcoal mb-4">Thank You!</h1>
                                        <p className="text-gray-600 mb-8 leading-relaxed">
                                                Your generous donation has been received. You will receive an email receipt shortly. Your support helps us continue our mission.
                                        </p>
                                        <Button 
                                                onClick={() => setLocation("/")}
